...
async function set(key, value, ttlSeconds) {
  if (ttlSeconds) return call(['SET', key, String(value), 'EX', String(ttlSeconds)]);
  return call(['SET', key, String(value)]);
}

async function sunionstore(destination, ...keys) {
  return call(['SUNIONSTORE', destination, ...keys]);
}
async function del(key) { return call(['DEL', key]); }

module.exports = { available, call, incr, lpush, ltrim, lrange, llen, hincrby, hgetall, sadd, scard, sunionstore, expire, get, set, del };
